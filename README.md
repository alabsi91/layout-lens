...
```
claude mcp add layout-lens -- npx -y -p layout-lens layout-lens-mcp
```

Codex:

```
codex mcp add layout-lens -- npx -y -p layout-lens layout-lens-mcp
```
...
```json
{
  "mcpServers": {
    "layout-lens": {
      "command": "npx",
      "args": ["-y", "-p", "layout-lens", "layout-lens-mcp"]
    }
  }
}
```
